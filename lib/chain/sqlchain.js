...
function SqlChain(config, conninfo) {
     ...
     */
    this.resetTimeout = function resetTimeout(timeout) {
     ...
     * @private
     */
    this.onTimeoutCallback = function onTimeoutCallback() {
        var _self = this;
     ...
     * @param status
     */
    this.onTransactionCallback = function onTransactionCallback(status) {
     ...
     * @private
     */
    this.onCheckChainCallback = function onCheckChainCallback() {
     ...
     * @param chains
     */
    this.onExecChainCallback = function onExecChainCallback(chains) {
     ...
     */
    this.setConnectionInfo = function (conninfo) {
     ...
     * 需要在执行sql之前开启。
     *
     * @param bool {Boolean} 如果不是布尔值，不设置事务开关
     */
    this.setTransaction = function(bool){
        if(typeof bool != 'boolean') { return; }

        if(this.config){
            this.config.transaction = bool
        }
    };
     ...
     * @returns {Function}
     */
    this.getMapper = function getMapper(name) {
     ...
     * @param cb {Function}
     */
    this.exec = function (sql, val, cb) {
        if (!this.isAvailable()) return this;

        if (!this.initTransition) {
            let transaction = false;
            // 判断是否开启事务,如果允许开启事务将开启事务
            if ('transaction' in this.config && this.config.transaction == true) {
                transaction = true;
            }
            process.nextTick(() => {
        ...
        if (sql instanceof Function) {
            ...
            if (val && val instanceof Function) callback = val;
            if (cb != undefined && !(val instanceof Function) && cb instanceof Function)
                callback = cb;
            this.chains.push({
                type: 'sql',
                args: [vals.sql, vals.values, callback]
            });
        } else {
            this.chains.push({
                type: 'sql',
     ...
     * @returns {Promise<any>}
     */
    this.exec.promise = function (sql, val) {
     ...
     * @returns {*}
     */
    this.findOne = function findOne(sql, val, cb) {
        var _self = this;
        ...
        this.exec(sql, val, cb._around(function (joinpoint) {
            // 修改获取到的参数, 只拿第一个对象的数据
     ...
     * @returns {Promise<any>}
     */
    this.findOne.promise = function (sql, val) {
        let _self = this.parent.findOne;
        let _parent = this.parent;
        return new Promise((resolve, reject) => {
            let _selfargs = [sql];
     ...
     * @private
     */
    this._exec = function _exec(sql, val, cb) {
    ...
    this.rollback = function (cb) {
        ...
        if (!this.isAvailable()) {
            ...
            if (this.getConnection()) {
                this.getConnection().rollback(cb);
            } else {
                cb()
            }
        }
    ...
    this.rollback.promise = function () {
     ...
     * @private
     */
    this._closeTimer = function _closeTimer() {
     ...
     * @param callback
     */
    this.end = function end(callback) {
        let _self = this;
        _self.hasend = true;
    ...
    this._throw = {err: null, data: null};
    ...
    // set the timer, automatically close the SsqlChain after the time,
