...
Ebatis.prototype.loadConfigFile = function loadConfigFile (filename) {
    ...
    if(filepath.endsWith('yml')){
        config = Yaml.load(path.resolve(this.$root, filename));
    } else if(filepath.endsWith('js')){
        config = require(path.resolve(this.$root, filename));
    } else if(filepath.endsWith('json')){
        config = JSON.parse(fs.readFileSync(path.resolve(this.$root, filename)).toString());
    }
...
/**
 * 将Ebatis的finish函数promise化
 * 在异步函数中异步调用完finish函数后，代码才会继续向下执行
 *
 * @example
 *
 *  (async function(){
 ...
 *
 *      let sqlchain = ebatis.SqlChainFactory.createSqlChain();
 *
 *      sqlchain.exec(funtion(){
 *          // code ...
 *      })
 *
 *      // login code
...
Ebatis.prototype.finish.promise = function(){
    ...
    return new Promise((resolve, reject)=>{
        ...
        _parent.on('finish', _parent._finish);
    });
};
