 ...
 * ### <when>
 *
 * 如果没有 test 属性 将忽略这个 when标签
 * 如果test中需要进行&& 或者 || 的逻辑，需要用and（代表&&） 或者 or（代表||）来替换
 *
 * when 标签相当于 逻辑判断中的if，
 * 如果 choose 标签中包含多个when标签，那么 从第二个when开始相当于逻辑判断中的else if
 *
 * ``` js
 *
 * <when test="name != null">
